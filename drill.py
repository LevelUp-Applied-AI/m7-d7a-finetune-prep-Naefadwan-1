...
# 1. Fix for datasets library: Pickler._batch_setitems signature change in Python 3.14
_original_batch_setitems = datasets.utils._dill.Pickler._batch_setitems
def _patched_batch_setitems(self, items, obj=None):
    return _original_batch_setitems(self, items)
datasets.utils._dill.Pickler._batch_setitems = _patched_batch_setitems
...
def make_dataset(csv_path: str, test_size: float, seed: int) -> DatasetDict:
    ...
    # Convert pandas DataFrame to HF Dataset (preserve_index=False is default for from_dict)
